...
| Screen Door Zombie | is initialized as a Buckethead Zombie | Hook `0x5225E6 -> 0x4002E0` maps incoming type 6 to 4 before storing `[Zombie+0x24]`. | `Zombie::ZombieInitialize`, `Lawn/Zombie.cpp:85`; `Zombie.h:90` maps `+0x24` to type and `ConstEnums.h` names 6 Door/4 Buckethead. | `ResolveZombieType`, `src/GameRules/Ruleset.cpp:338-344`, called before initialization at `src/Lawn/Zombie.cpp:146`. |
...
The full-reference audit covered 93 documented changes at the document's existing granularity: 22 direct table fields, 11 initial behavior rows, 24 batch-2 behavior rows, 28 batch-3 immediate replacements, and 8 batch-4 behavior rows. Every group now has an A/B/C path from executable evidence, through reference C++ semantics, to the Portable implementation. It found four additional translation-error groups in already-implemented rules and corrected them in code and tests:
