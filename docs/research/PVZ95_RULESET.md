...
The full-reference audit now covers 103 documented changes at the document's existing granularity: 22 direct table fields, 11 initial behavior rows, 24 batch-2 behavior rows, 28 batch-3 immediate replacements, 8 batch-4 behavior rows, 7 batch-5 Board behavior groups, and 3 batch-6 behavior groups. Every group has an A/B/C path from executable evidence, through reference C++ semantics, to the Portable implementation. The audit found five translation-error groups in already-implemented rules and corrected them in code and tests:
