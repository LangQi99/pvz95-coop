...
LAN discovery uses non-blocking IPv4 UDP on port `43095` by default. A client broadcasts a versioned query; a host replies directly with its session ID, player count, ruleset ID, and reliable game port. The reliable IPv4 TCP listener uses port `43096` by default, and the host displays its actual bound port in the lobby status. Tests may explicitly request an ephemeral TCP port to avoid parallel-run collisions. Discovery is kept separate from the reliable gameplay connection so cursor/action traffic never depends on broadcast delivery.
...
`LanCoordinator` owns discovery and reliable-session lifetimes. The main-menu **Host LAN** control starts the fixed-port listener, **Join Room** resolves an editable IPv4 address or DNS host name and connects to its TCP endpoint, and **Auto Search** uses UDP discovery. The regular application update loop polls all paths without blocking rendering and gameplay input. A host continually refreshes its discovery offer as players join, while an automatically searching client discovers a compatible non-full room and advances through search, handshake, and connected states. A direct client skips discovery, accepts the host's first nonzero session ID during the validated `Welcome`, and retains the same ruleset/player/tick validation as a discovered connection.
