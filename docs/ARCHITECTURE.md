...
`LanCoordinator` owns discovery and reliable-session lifetimes. The main-menu **Host LAN** control starts the fixed-port listener, **Join Room** connects directly to an editable numeric IPv4/TCP endpoint, and **Auto Search** uses UDP discovery. The regular application update loop polls all paths without blocking rendering or input. A host continually refreshes its discovery offer as players join, while an automatically searching client discovers a compatible non-full room and advances through search, handshake, and connected states. A direct client skips discovery, accepts the host's first nonzero session ID during the validated `Welcome`, and retains the same ruleset/player/tick validation as a discovered connection.
